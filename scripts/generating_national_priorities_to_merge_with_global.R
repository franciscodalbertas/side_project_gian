...
# normalize values per country!! then create deciles!!
...
df_national <- df_national %>% mutate(
  regen_05 = case_when(
    nat_regen >= 50 ~ 1,
    nat_regen < 50  ~ 0,
    TRUE ~ NA_real_  ),
...
df_national$opp_cost <- df_national$opp_cost *100
df_national$opp_cost <- df_national$opp_cost * df_national$area_restorable_km2

# calculate carbon in km2!
df_national$carbon <- df_national$carbon *100
df_national$carbon <- df_national$carbon * df_national$area_restorable_km2



#---- calculate total restorable area (global) ----

total_restorable <- df_national %>%
  summarise(total_restorable_km = sum(area_restorable_km2))%>%
  pull()


#calculate total restorable area by country
tot_restor_country <- df_national %>%
...
# nat. regen
# 50%
top30_biod_50 <- make_priority_table(df_national, criteria = "biodiversity",threshold = "05") 
top30_carbon_50 <- make_priority_table(df_national, criteria = "carbon",threshold = "05") 
top30_costs_50 <- make_priority_table(df_national, criteria = "opp_cost",threshold = "05")

# 40%
top30_biod_40 <- make_priority_table(df_national, criteria = "biodiversity",threshold = "04") 
top30_carbon_40 <- make_priority_table(df_national, criteria = "carbon",threshold = "04") 
top30_costs_40 <- make_priority_table(df_national, criteria = "opp_cost",threshold = "04")

# 60%
top30_biod_60 <- make_priority_table(df_national, criteria = "biodiversity",threshold = "06") 
top30_carbon_60 <- make_priority_table(df_national, criteria = "carbon",threshold = "06") 
top30_costs_60 <- make_priority_table(df_national, criteria = "opp_cost",threshold = "06")
...
full_data <- full_data  %>%
  ...
  mutate(scale = "national")




write.csv(full_data,"full_data_to_combine_with_global.csv",row.names = F)
